...
kelp_longer %>% 
    filter(site_name == 'Dark Gulch') %>%
    ggplot() + 
    geom_pointrange(aes(
      x = year, y = mean, group = zone, color = zone, 
      ymin = mean - se, ymax = mean + se
      ), alpha = 0.5, size = 0.3) + 
    geom_bar(aes(x = year, y = fit, fill = ifelse(fit >= 6.6)), 
    ...
    labs(y = 'log of kelp density', title = 'Dark Gulch')
...
for (i in sites) {
  plot <- kelp_longer %>% 
    ...
    geom_bar(aes(x = year, y = fit, colour = (fit >= 6.6)), 
             stat = 'identity', position = 'dodge', 
             fill = 'blue', alpha = 0.3) + 
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 90, size = 8), 
