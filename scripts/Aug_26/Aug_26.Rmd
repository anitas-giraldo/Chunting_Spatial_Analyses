...
ggplot() +
  geom_bar(data = filter(kelp_density, site_name == 'MacKerricher North'),
           aes(x = year, y = fit,
               fill = ifelse(!is.na(fit) & fit >= 6.6, 'YES', 'NO')),
           stat = 'identity', position = 'dodge', alpha = 0.2) + 
  geom_pointrange(data = filter(kelp_area3, site_name == 'MacKerricher North'),
                  aes(x = year, y = mean_area, 
                      ymin = mean_area - se_area, ymax = mean_area + se_area),
                  alpha = 0.5, size = 0.3) +
  geom_line(data = filter(kelp_area3, site_name == 'MacKerricher North'),
  ...
  # geom_bar(data = filter(kelp_density, site_name == 'Fort Ross'),
  #          aes(x = year, y = fit, 
  #              fill = ifelse(!is.na(fit) & fit >= 6.6, 'YES', 'NO')),
  #          stat = 'identity', position = 'dodge', alpha = 0.2) + 
  # theme_bw() + 
