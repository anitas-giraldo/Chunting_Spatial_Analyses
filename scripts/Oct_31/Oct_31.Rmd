...
# fit the data
fits120 <- predict.gam(gam120, newdata = train.gam, type = 'response', se.fit = T)

predicts.all <- train.gam %>%
  data.frame(fits120) %>%
  ungroup()
...
fits120 <- predict.gam(gam120, newdata = test.gam, type = 'response', se.fit = T)

predicts.all <- test.gam %>%
  data.frame(fits120) %>%
  ungroup()
