...
depth.90 <- rast(paste0(depth.dir, '/depth_mean_nc.all_wInterp_90m_30m.tif'))
depth.120 <- rast(paste0(depth.dir, '/depth_mean_nc.all_wInterp_120m_30m.tif'))
...
depth.1500 <- rast(paste0(depth.dir, '/depth_mean_nc.all_wInterp_1500m_30m.tif'))
...
rock.90 <- rast(paste0(sub.dir, '/rock_NC_90res_30m.tif'))
rock.120 <- rast(paste0(sub.dir, '/rock_NC_120res_30m.tif'))
...
rock.1500 <- rast(paste0(sub.dir, '/rock_NC_1500res_30m.tif'))

rock <- rock.120

rock1 <- terra::project(rock, crs1) 
...
# disaggregate
# max_nit5 <- disagg(max_nit2, fact = 18) # 300m
# max_nit5 <- disagg(max_nit2, fact = 9) # 600m
# max_nit5 <- disagg(max_nit2, fact = 6) # 900m
# max_nit5 <- disagg(max_nit2, fact = 36) # 150m
max_nit5 <- disagg(max_nit2, fact = 45) # 120m
# max_nit5 <- disagg(max_nit2, fact = 4) # 1500m
# max_nit5 <- disagg(max_nit2, fact = 59) # 90m

# resample predictors to bathy
max_nit3 <- resample(max_nit5, d2)
...
n.files <- list.files(wave.dir, pattern = '.tif$', full.name = TRUE)
...
# disaggregate 
# whmax.stack5 <- disagg(whmax.stack2, fact = 4) # 300m
# whmax.stack5 <- disagg(whmax.stack2, fact = 2) # 600m
# whmax.stack5 <- whmax.stack2 # 900m
# whmax.stack5 <- disagg(whmax.stack2, fact = 7) # 150m
whmax.stack5 <- disagg(whmax.stack2, fact = 9) # 120m
# whmax.stack5 <- whmax.stack2 # 1500m
# whmax.stack5 <- disagg(whmax.stack2, fact = 12) # 90m

# resample predictors to bathy
whmax.stack3 <- resample(whmax.stack5, d2)
...
u.files <- list.files(urch.dir, pattern = '.tif', full.name = TRUE)

# list names to load onto the Environment
names.list <- list.files(urch.dir, pattern = '.tif$')
names.list <- str_replace(names.list, '.tif$', '')
...
# crop to NC
urchin.stack2 <- crop(urchin.stack, n.extent)

# disaggregate/aggregate
# urchin.stack5 <- urchin.stack2 # 300m
# urchin.stack5 <- aggregate(urchin.stack2, fact = 2, fun = mean, na.rm = TRUE) # 600m
# urchin.stack5 <- aggregate(urchin.stack2, fact = 3, fun = mean, na.rm = TRUE) # 900m
# urchin.stack5 <- disagg(urchin.stack2, fact = 2) # 150m
urchin.stack5 <- disagg(urchin.stack2, fact = 2.5) # 120m
# urchin.stack5 <- aggregate(urchin.stack2, fact = 5, fun = mean, na.rm = TRUE) # 1500m
# urchin.stack5 <- disagg(urchin.stack2, fact = 3) # 90m

# resample predictors to bathy
urchin.stack3 <- resample(urchin.stack5, d2)

# mask predictors to bathy
urchin.stack4 <- mask(urchin.stack3, d2)
```
...
# outputs dir
o2.dir <- '/Volumes/Chunting HD/Git_Repositories/Chunting_Spatial_Analyses/spatial_data'
# o2.dir <- here('spatial_data')

preds.dir <- paste(o2.dir, 'sp_predictions_120m', sep ='/')
preds.dir

# output for rasters scaled by rock 
rock.preds.dir <- paste(o2.dir, "sp_predictions_120m_rock", sep ='/')
rock.preds.dir
...
for (i in 1:length(year.list)) {
  ...
  preds1 <- c(urchin.stack4[[i]], env.raster)
  ...
  writeRaster(year.prediction, paste(preds.dir, name.raster, sep = '/'), overwrite = TRUE)
  ...
  writeRaster(year.prediction2, paste(rock.preds.dir, name.raster.rock, sep = '/'), overwrite = TRUE)
}
```