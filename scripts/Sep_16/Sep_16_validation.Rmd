...
w.dir <- here()
d.dir <- here('data')
r1.dir <- here('spatial_data/sp_predictions_300m')
r2.dir <- here('spatial_data/sp_predictions_150m')
r3.dir <- here('spatial_data/sp_predictions_600m')
r4.dir <- here('spatial_data/sp_predictions_900m')
r5.dir <- here('spatial_data/sp_predictions_120m')
r6.dir <- here('spatial_data/sp_predictions_1500m')
...
# write.csv(merge_df, file.path(d.dir, 'NC_kelp_density_predictions_300m.csv'), row.names = FALSE)
```
...
# write.csv(merge_df, file.path(d.dir, 'NC_kelp_density_predictions_150m.csv'), row.names = FALSE)
```
...
# write.csv(merge_df, file.path(d.dir, 'NC_kelp_density_predictions_600m.csv'), row.names = FALSE)
```
...
for (i in c(2006:2021)) {
  rast <- rast(paste0(r4.dir, paste0('/', i, '_Log_Nereo_NC.tif')))
  ext <- terra::extract(rast, vect(site_shp$geometry)) %>%
    mutate(site_name = site$site_name, year = as.factor(i), .before = fit) %>%
    dplyr::select(-ID)
  pred <- rbind(pred, ext)
...
# write.csv(merge_df, file.path(d.dir, 'NC_kelp_density_predictions_900m.csv'), row.names = FALSE)
```
...
for (i in c(2006:2021)) {
  rast <- rast(paste0(r5.dir, paste0('/', i, '_Log_Nereo_NC.tif')))
  ext <- terra::extract(rast, vect(site_shp$geometry)) %>%
    mutate(site_name = site$site_name, year = as.factor(i), .before = fit) %>%
    dplyr::select(-ID)
  pred <- rbind(pred, ext)
...
# write.csv(merge_df, file.path(d.dir, 'NC_kelp_density_predictions_1500m.csv'), row.names = FALSE)
```
